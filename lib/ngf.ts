...
export async function getNgfContent(): Promise<NgfSiteContent> {
  try {
    ...
    // content API on every request. The NGF push handler busts this instantly
    // on publish via /api/revalidate. If that ping never lands, content still
    // refreshes within 60s. See NGF-STANDARDS.md.
